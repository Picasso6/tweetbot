...
#1.ENVOIE UN MESSAGE SUR LE MUR
#login_twitter.update('BONJOUR JE SUIS UN BOT TRO STYLÉ !!!!')

#2.ENVOIE LA PHRASE ET LE HASHTAG À 5 JOURNALISTES AU HASARD (RANDOM_MAIL(ARR))
# 5.times do
#   login_twitter.update(random_mail(journalope))
# end

#3.RETROUVE LE HASHTAG + LE LIKE
...
def follow_monde
  login_twitter.search("#bonjour_monde").take(20).each do |tweet|
    puts tweet.text
    sleep 0.7
...
def follow_like_stream
  topics = ["#bonjour_monde"]
  login_twitter_stream.filter(track: topics.join(",")) do |object|
     if object.is_a?(Twitter::Tweet)
        puts object.text
  			login_twitter.favorite(object)
        login_twitter.follow(object.user)
      end
  end
end
follow_like_stream
